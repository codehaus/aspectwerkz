...
public class AspectWerkzAspectModel implements AspectModel, Constants, TransformationConstants {
    ...
    public void initialize(CompilationInfo.Model model) {
        ;
    }
    ...
    public void createMandatoryMethods(ClassWriter cw, AbstractJoinPointCompiler compiler) {
        createCustomProceedMethods(cw, compiler);
    }
    ...
    /**
     * Create one jp field per aspect
     *
     * @param cw
     * @param aspectInfo
     * @param joinPointClassName
     */
    public void createAspectReferenceField(ClassWriter cw, AspectInfo aspectInfo, String joinPointClassName) {
     ...
     * @param joinPointClassName
     */
    public void createAndStoreStaticAspectInstantiation(CodeVisitor cv, AspectInfo aspectInfo, String joinPointClassName) {
        String aspectClassSignature = aspectInfo.getAspectClassSignature();
        ...
        for (Iterator it = m_customProceedMethodStructs.iterator(); it.hasNext();) {
            ...
            for (int i = 1; i < customProceedStruct.adviceToTargetArgs.length; i++) {
                ...
                if (targetArg >= 0) {
                    ...
                    cv.visitFieldInsn(PUTFIELD, compiler.m_joinPointClassName, CALLEE_INSTANCE_FIELD_NAME, compiler.m_calleeClassSignature);
                    ...
                    cv.visitFieldInsn(PUTFIELD, compiler.m_joinPointClassName, CALLER_INSTANCE_FIELD_NAME, compiler.m_callerClassSignature);
                } else {
                    ;//skip it
                }
            }
