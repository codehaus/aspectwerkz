...
public class SnapshotScheduler extends TimerTask {
     ...
     * @param listener If non-null, the listener will be called each time a snapshot has been made.
     */
    public SnapshotScheduler(Prevayler prevayler, long delay, long period, Listener listener) {
        this.prevayler = prevayler;
        if (listener != null) {
            listenerList.add(listener);
        }
    ...
    private void snapshotStarted() {
        long prevaylerDate = ((ClockedSystem)prevayler.prevalentSystem()).clock().time().getTime();
