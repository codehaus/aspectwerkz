...
public class AddInterfacePreProcessor implements ClassPreProcessor
{
    ...
    public byte[] preProcess(String klass, byte[] abyte, ClassLoader caller)
    {
        try
        {
            ...
            // build the ClassGen
            ClassParser parser = new ClassParser(new ByteArrayInputStream(abyte),
                    "<generated>"); //@todo is this needed _"+klass+">");
            ...
            try
            {
                cg.getJavaClass().dump("_dump/" + klass.replace('.', '/')
                    + ".class");
            }
            catch (Exception e)
            {
