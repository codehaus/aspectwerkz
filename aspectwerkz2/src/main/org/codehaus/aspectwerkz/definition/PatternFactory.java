...
public class PatternFactory
{
        ...
        final String packageName)
    {
        ...
        try
        {
            ...
            {
                classPattern = classPattern.substring(0,
                        classPattern.length() - 1);
                isHierarchical = true;
            }

            return new PatternTuple(null, classPattern, classPattern,
                isHierarchical);
        }
        catch (Exception e)
        {
            throw new DefinitionException("class pattern is not well formed ["
                + pattern + ']');
        }
    }
     ...
     */
    public static String createClassPattern(final String pattern,
        final String packageName)
    {
        if (pattern == null)
        {
        ...
        final String packageName)
    {
        ...
        try
        {
            ...
            String classNameWithMethodName = pattern.substring(indexFirstSpace,
                    pattern.indexOf('(')).trim();
            String parameterTypes = pattern.substring(pattern.indexOf('('),
                    pattern.length()).trim();
            int indexLastDot = classNameWithMethodName.lastIndexOf('.');

            final String methodPattern = classNameWithMethodName.substring(indexLastDot
                    + 1, classNameWithMethodName.length()).trim();
            String classPattern = packageName
                + classNameWithMethodName.substring(0, indexLastDot);
            ...
            return new PatternTuple(null, classPattern,
                memberPattern.toString(), isHierarchical);
        }
        catch (Exception e)
        {
     ...
     * @return a tuple of the class patterns and the execution pattern
     */
    public static PatternTuple createConstructorPatternTuple(
        final String pattern, final String packageName)
    {
        if (pattern == null)
        {
            throw new IllegalArgumentException("pattern can not be null");
        ...
        try
        {
            String classNameWithMethodName = pattern.substring(0,
                    pattern.indexOf('(')).trim();
            String parameterTypes = pattern.substring(pattern.indexOf('('),
                    pattern.length()).trim();
            int indexLastDot = classNameWithMethodName.lastIndexOf('.');
        ...
        catch (Exception e)
        {
            throw new DefinitionException(
                "constructor pattern is not well formed [" + pattern + ']');
        }
    }
     ...
     */
    public static String createMethodPattern(final String pattern,
        final String packageName)
    {
        if (pattern == null)
        {
        ...
        final String packageName)
    {
        ...
        try
        {
            ...
            String classNameWithFieldName = pattern.substring(indexFirstSpace,
                    pattern.length()).trim();
            int indexLastDot = classNameWithFieldName.lastIndexOf('.');

            final String fieldPattern = classNameWithFieldName.substring(indexLastDot
                    + 1, classNameWithFieldName.length()).trim();
            String classPattern = packageName
                + classNameWithFieldName.substring(0, indexLastDot).trim();
     ...
     */
    public static String createFieldPattern(final String pattern,
        final String packageName)
    {
        if (pattern == null)
        {
        ...
        final String packageName)
    {
        ...
        try
        {
            String classAndMethodName = pattern.substring(0,
                    pattern.indexOf('#')).trim();
            final String exceptionName = pattern.substring(pattern.indexOf('#')
                    + 1).trim();
            int indexFirstSpace = classAndMethodName.indexOf(' ');
            final String returnType = classAndMethodName.substring(0,
                    indexFirstSpace + 1);
            String classNameWithMethodName = classAndMethodName.substring(indexFirstSpace,
                    classAndMethodName.indexOf('(')).trim();
            final String parameterTypes = classAndMethodName.substring(classAndMethodName
                    .indexOf('('), classAndMethodName.length()).trim();
            int indexLastDot = classNameWithMethodName.lastIndexOf('.');
            final String methodPattern = classNameWithMethodName.substring(indexLastDot
                    + 1, classNameWithMethodName.length()).trim();
            String classPattern = packageName
        ...
        {
            throw new DefinitionException("throws pattern is not well formed ["
                + pattern + ']');
        }
    }
     ...
     */
    public static String createThrowsPattern(final String pattern,
        final String packageName)
    {
        if (pattern == null)
        {
        ...
        try
        {
            ...
            final String returnType = classAndMethodName.substring(0,
                    indexFirstSpace + 1);
            String classNameWithMethodName = classAndMethodName.substring(indexFirstSpace,
                    classAndMethodName.length()).trim();
     ...
     */
    public static PatternTuple createCallPatternTuple(final int type,
        String pattern, final String packageName)
    {
        if (pattern == null)
        {
            throw new IllegalArgumentException("pattern can not be null");
        ...
        try
        {
            ...
            String callerClassPattern = packageName
                + pattern.substring(0, pattern.indexOf('-')).trim();

            if (callerClassPattern.endsWith("+"))
            {
                callerClassPattern = callerClassPattern.substring(0,
                        callerClassPattern.length() - 1);
                isHierarchical = true;
            }

            String calleePattern = pattern.substring(pattern.indexOf('>') + 1)
                                          .trim();
            ...
            {
                String classNameWithMethodName = calleePattern.substring(0,
                        calleePattern.indexOf('(')).trim();
                String parameterTypes = calleePattern.substring(calleePattern
                        .indexOf('('), calleePattern.length()).trim();
                int indexLastDot = classNameWithMethodName.lastIndexOf('.');

                calleeMethodPattern = classNameWithMethodName.substring(indexLastDot
                        + 1, classNameWithMethodName.length()).trim();
                calleeClassPattern = packageName
                    + classNameWithMethodName.substring(0, indexLastDot);

                if (calleeClassPattern.endsWith("+"))
                {
                    calleeClassPattern = calleeClassPattern.substring(0,
                            calleeClassPattern.length() - 1);
            ...
            {
                String returnType = calleePattern.substring(0,
                        indexFirstSpace + 1);
                String classNameWithMethodName = calleePattern.substring(indexFirstSpace,
                        calleePattern.indexOf('(')).trim();
                String parameterTypes = calleePattern.substring(calleePattern
                        .indexOf('('), calleePattern.length()).trim();
                int indexLastDot = classNameWithMethodName.lastIndexOf('.');
                ...
                calleeMethodPattern = returnType + calleeMethodPattern
                    + parameterTypes;
            }
            ...
            return new PatternTuple(callerClassPattern, calleeClassPattern,
                buf.toString(), isHierarchical, isHierarchicalCallee);
        }
        catch (Exception e)
        {
            throw new DefinitionException(
                "caller side pattern is not well formed [" + pattern + ']');
        }
    }
     ...
     */
    public static String createCallPattern(String pattern,
        final String packageName)
    {
        if (pattern == null)
        {
        ...
        try
        {
            ...
            String parameterTypes = calleePattern.substring(calleePattern
                    .indexOf('('), calleePattern.length()).trim();
            int indexLastDot = classNameWithMethodName.lastIndexOf('.');
            String calleeMethodPattern = classNameWithMethodName.substring(indexLastDot
                    + 1, classNameWithMethodName.length()).trim();
            String calleeClassPattern = packageName
                + classNameWithMethodName.substring(0, indexLastDot);
