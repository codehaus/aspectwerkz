...
public class AsmAttributeEnhancer implements AttributeEnhancer
{
    ...
    public void insertClassAttribute(final Object attribute)
    {
        if (m_writer == null)
        {
            throw new IllegalStateException(
                "attribute enhancer is not initialized");
        }
        ...
        m_reader.accept(new AttributeClassAdapter(m_writer, serializedAttribute)
            {
                public void visit(final int access, final String name,
     ...
     * @param attribute the attribute
     */
    public void insertFieldAttribute(final JavaField field,
                    ...
                    final String desc, final Object value, final Attribute attrs)
                {
                    if (name.equals(field.getName()))
                    {
                        cv.visitField(access, name, desc, value,
                            new CustomAttribute(serializedAttribute));
                    }
     ...
     * @param attribute the attribute
     */
    public void insertMethodAttribute(final JavaMethod method,
        final Object attribute)
    {
        if (m_writer == null)
        {
        ...
        {
            methodParamTypes[i] = TypeConverter.convertTypeToJava(method
        ...
        m_reader.accept(new AttributeClassAdapter(m_writer, serializedAttribute)
            {
                    ...
                    final String[] exceptions, final Attribute attrs)
                {
                    if (name.equals(method.getName())
                        && Arrays.equals(methodParamTypes,
                            DescriptorUtil.convertToJavaFormat(desc)))
                    {
                        cv.visitMethod(access, name, desc, exceptions,
                            new CustomAttribute(serializedAttribute));
                    }

                    return super.visitMethod(access, name, desc, exceptions,
                        attrs);
                }
            }, false);
    }
    ...
    public void write(final String destDir)
    {
        try
        {
            ...
            FileOutputStream os = new FileOutputStream(destDir + File.separator
                    + m_classFileName);
    ...
    public String[] getNearestInterfacesInHierarchy(final String innerClassName)
    {
        ...
        {
            throw new RuntimeException(
                "could not load mixin for mixin implicit interface: "
                + e.toString());
        }
        catch (NoClassDefFoundError er)
        {
            ...
            throw new RuntimeException(
                ...
                + innerClassName + " due to: " + er.toString());
        }
    }
    ...
    private static class AttributeClassAdapter extends ClassAdapter
    {
        ...
        public AttributeClassAdapter(final ClassVisitor cv,
            final byte[] serializedAttribute)
        {
            super(cv);
            m_serializedAttribute = serializedAttribute;
        }

        public void visit(int i, String s, String s1, String[] strings,
            String s2)
        {
        ...
        public void visitField(int i, String s, String s1, Object o,
            Attribute attribute)
        {
            super.visitField(i, s, s1, o, attribute);
        }

        public CodeVisitor visitMethod(int i, String s, String s1,
            String[] strings, Attribute attribute)
        {
