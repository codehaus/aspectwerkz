...
public class AsmAttributeExtractor implements AttributeExtractor
{
    ...
    public Object[] getClassAttributes()
    {
        if (m_reader == null)
        {
            throw new IllegalStateException(
                "attribute extractor is not initialized");
        }
        ...
        m_reader.accept(new ClassAdapter(m_writer)
            {
                public void visitAttribute(final Attribute attribute)
                {
                    if (attribute instanceof CustomAttribute)
                    {
                        ...
                        try
                        {
                            classAttributes.add(new ObjectInputStream(
     ...
     */
    public Object[] getMethodAttributes(final String methodName,
                    ...
                    final String[] exceptions, final Attribute attribute)
                {
                            ...
                            DescriptorUtil.convertToJavaFormat(desc)))
                    {
                        if (attribute instanceof CustomAttribute)
                        {
                            ...
                            try
                            {
                                methodAttributes.add(new ObjectInputStream(
                    ...
                    final Attribute attribute)
                {
                    if (name.equals(fieldName))
                    {
                        if (attribute instanceof CustomAttribute)
                        {
                            ...
                            try
                            {
                                fieldAttributes.add(new ObjectInputStream(
                                        new ByteArrayInputStream(bytes))
                                    .readObject());
                            }
                            catch (Exception e)
