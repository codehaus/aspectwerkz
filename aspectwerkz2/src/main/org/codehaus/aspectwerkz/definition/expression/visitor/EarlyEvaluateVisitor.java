...
public class EarlyEvaluateVisitor implements ExpressionParserVisitor {
    ...
    public Object visit(OrNode node, Object data) {
        ...
        if (lhs.booleanValue()) {
            return Boolean.TRUE;//standard OR rule
        }
    ...
    public Object visit(AndNode node, Object data) {
        Boolean lhs = (Boolean)node.jjtGetChild(0).jjtAccept(this, data);
        if (!lhs.booleanValue()) {
            return Boolean.FALSE;//standard AND rule
        }
        Boolean rhs = (Boolean)node.jjtGetChild(1).jjtAccept(this, data);
        return rhs;
    }
    ...
    public Object visit(NotNode node, Object data) {
        // ignore the NOT since we cannot rely on the match result no matter
        // it is true or false
        // match(foo.p(..))(!baz && foo) => !FALSE && TRUE
