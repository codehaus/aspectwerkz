...
public class MethodExecutionTransformer implements Transformer
{
        ...
        throws Exception
    {
        ...
        m_joinPointIndex = TransformationUtil.getJoinPointIndex(klass
                .getCtClass()); // TODO not thread safe
        ...
        for (Iterator methodsIt = sortedMethods.iterator();
            methodsIt.hasNext();)
        {
            CtMethod method = (CtMethod) methodsIt.next();
            MethodMetaData methodMetaData = JavassistMetaDataMaker
                .createMethodMetaData(method);
            ...
            tuple.setStatus(methodFilter(definitions, classMetaData,
        ...
        for (Iterator i = sorteMethodTuples.iterator(); i.hasNext();)
        {
            ...
            final String prefixedMethodName = TransformationUtil
                .getPrefixedMethodName(method.getName(), tuple.getSequence(),
                    ctClass.getName().replace('/', '.'));
            ...
            if (JavassistHelper.hasMethod(ctClass, prefixedMethodName))
            {
                ...
                if (JavassistHelper.isAnnotatedNotEmpty(wrapperMethod))
                {
                    ...
                    CtMethod emptyWrapperMethod = JavassistHelper
                        .createEmptyWrapperMethod(ctClass, method,
                            tuple.getSequence());
        ...
        final int methodSequence)
    {
        ...
        String prefixedMethodName = TransformationUtil.getPrefixedMethodName(ctMethod
                .getName(), methodSequence, cg.getName());
     ...
     * @return boolean
     */
    private int methodFilter(final SystemDefinition definition,
        final ClassMetaData classMetaData, final MethodMetaData methodMetaData,
    ...
    private int methodFilter(final List definitions,
        final ClassMetaData classMetaData, final MethodMetaData methodMetaData,
        final CtMethod method)
    {
        if (Modifier.isAbstract(method.getModifiers())
        ...
        for (Iterator defs = definitions.iterator(); defs.hasNext();)
        {
            if (STATUS_HASPOINTCUT == methodFilter(
                    (SystemDefinition) defs.next(), classMetaData,
                    methodMetaData, method))
            {
                return STATUS_HASPOINTCUT;
            }
     ...
     * @return boolean
     */
    private boolean methodHasNoPointcut(final SystemDefinition definition,
        final ClassMetaData classMetaData, final MethodMetaData methodMetaData,
        final CtMethod method)
    {
        if (Modifier.isAbstract(method.getModifiers())
