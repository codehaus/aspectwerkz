...
public class ClassLoaderPatcher
{
    ...
    static byte[] getPatchedClassLoader(String preProcessorName)
    {
        ...
        {
            throw new Error("failed to read java.lang.ClassLoader: "
                + e.toString());
        }
        ...
        if (preProcessorName != null)
        {
            try
            {
                ClassLoaderPreProcessor clpi = (ClassLoaderPreProcessor) Class.forName(preProcessorName)
                                                                              .newInstance();
            ...
            {
                System.err.println(
                    "failed to instrument java.lang.ClassLoader: preprocessor not found");
                e.printStackTrace();
            }
        }
     ...
     */
    private static void redefineClass(VirtualMachine vm, String className,
        byte[] bytes)
    {
        // determine if VM support class HotSwap with introspection
        try
        {
            Method canM = VirtualMachine.class.getMethod("canRedefineClasses",
                    new Class[] {  });

            if (((Boolean) canM.invoke(vm, new Object[] {  })).equals(
                    Boolean.FALSE))
            {
                throw new Error(
            ...
            Method doM = VirtualMachine.class.getMethod("redefineClasses",
                    new Class[] { Map.class });
    ...
    public static VirtualMachine hotswapClassLoader(String preProcessorName,
        String transport, String address)
    {
     ...
     */
    public static VirtualMachine hotswapClassLoader(String preProcessorName,
        String transport, String address, int secondsToWait)
    {
        String name = null;
        ...
        for (Iterator i = Bootstrap.virtualMachineManager().attachingConnectors()
                                   .iterator(); i.hasNext();)
        {
        ...
        try
        {
            ...
            ConnectException vmConnectionRefused = new ConnectException(
                    "should not appear as is");
            ...
            redefineClass(vm, "java.lang.ClassLoader",
                getPatchedClassLoader(preProcessorName));

            return vm;
        }
        catch (IllegalConnectorArgumentsException e)
        {
            System.err.println("failed to attach to VM (" + transport + ", "
                + address + "):");
            e.printStackTrace();
