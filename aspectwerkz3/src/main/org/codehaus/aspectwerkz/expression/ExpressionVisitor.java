...
public class ExpressionVisitor implements ExpressionParserVisitor
{
     ...
     */
    public ExpressionVisitor(final String expression, final String namespace,
        final ASTRoot root)
    {
        m_root = root;
    ...
    public Object visit(ASTExecution node, Object data)
    {
        ...
        {
            return node.jjtGetChild(0).jjtAccept(this,
                context.getReflectionInfo());
        }
        else
        {
            return Boolean.FALSE;
    ...
    public Object visit(ASTMethodPattern node, Object data)
    {
        ...
        { // TODO: is this safe check needed?

            MethodInfo methodInfo = (MethodInfo) data;

            if (node.getMethodNamePattern().matches(methodInfo.getName())
                && ClassInfoHelper.matchType(node.getDeclaringTypePattern(),
                    methodInfo.getDeclaringType())
                && ClassInfoHelper.matchType(node.getReturnTypePattern(),
    ...
    public Object visit(ASTConstructorPattern node, Object data)
    {
        ...
        { // TODO: is this safe check needed?
            ...
            if (ClassInfoHelper.matchType(node.getDeclaringTypePattern(),
                    constructorMetaData.getDeclaringType())
    ...
    public Object visit(ASTFieldPattern node, Object data)
    {
        ...
        { // TODO: is this safe check needed?
            ...
            if (node.getFieldNamePattern().matches(fieldInfo.getName())
                && ClassInfoHelper.matchType(node.getDeclaringTypePattern(),
                    fieldInfo.getDeclaringType())
                && ClassInfoHelper.matchType(node.getFieldTypePattern(),
                    fieldInfo.getType()) && visitAttributes(node, fieldInfo)
    ...
    public Object visit(ASTModifier node, Object data)
    {
        ...
        if (((modifierPattern & Modifier.PUBLIC) != 0)
            && ((modifiersToMatch & Modifier.PUBLIC) == 0))
        {
            return Boolean.FALSE;
        }

        if (((modifierPattern & Modifier.PROTECTED) != 0)
            && ((modifiersToMatch & Modifier.PROTECTED) == 0))
        {
            return Boolean.FALSE;
        }

        if (((modifierPattern & Modifier.PRIVATE) != 0)
            && ((modifiersToMatch & Modifier.PRIVATE) == 0))
        {
            return Boolean.FALSE;
        }

        if (((modifierPattern & Modifier.STATIC) != 0)
            && ((modifiersToMatch & Modifier.STATIC) == 0))
        {
            return Boolean.FALSE;
        }

        if (((modifierPattern & Modifier.SYNCHRONIZED) != 0)
            && ((modifiersToMatch & Modifier.SYNCHRONIZED) == 0))
        {
            return Boolean.FALSE;
        }

        if (((modifierPattern & Modifier.FINAL) != 0)
            && ((modifiersToMatch & Modifier.FINAL) == 0))
        {
            return Boolean.FALSE;
        }

        if (((modifierPattern & Modifier.TRANSIENT) != 0)
            && ((modifiersToMatch & Modifier.TRANSIENT) == 0))
        {
            return Boolean.FALSE;
        }

        if (((modifierPattern & Modifier.VOLATILE) != 0)
            && ((modifiersToMatch & Modifier.VOLATILE) == 0))
        {
            return Boolean.FALSE;
        }

        if (((modifierPattern & Modifier.STRICT) != 0)
            && ((modifiersToMatch & Modifier.STRICT) == 0))
        {
            return Boolean.FALSE;
        }
    ...
    protected boolean visitAttributes(SimpleNode node, ReflectionInfo metaData)
    {
        ...
        if (nrChildren != 0)
        {
            for (int i = 0; i < nrChildren; i++)
            {
                ...
                {
                    if (Boolean.TRUE.equals(child.jjtAccept(this,
        ...
        ClassInfo[] parameterTypes)
    {
        ...
        { // has nodes
            ...
            for (Iterator iterator = parameterNodes.iterator();
                iterator.hasNext();)
            {
                ...
                if (Boolean.TRUE.equals(parameter.jjtAccept(this,
                            parameterTypes[j++])))
                {
                    continue;
                }
        ...
        { // no nodes but parameters to match
