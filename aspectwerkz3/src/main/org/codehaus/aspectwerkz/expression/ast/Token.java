...
public class Token {
     ...
     * one, this field is setClass to null. This is true only if this token is also a regular token.
     ...
     * field is setClass to null. When there are more than one such special token, this field refers
     * to the last of these special tokens, which in turn refers to the next previous special token
     ...
     * want to create if ofKind is ID, simlpy add something like : <p/>case MyParserConstants.ID :
     * return new IDToken(); <p/>to the following switch statement. Then you can cast matchedToken
