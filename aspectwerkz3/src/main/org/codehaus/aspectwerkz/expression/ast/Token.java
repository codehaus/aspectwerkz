...
public class Token
{
     ...
     * input stream, or if the token manager has not read tokens beyond this one, this field is setClass to null.  This
     * is true only if this token is also a regular token.  Otherwise, see below for a description of the contents of
     * this field.
     */
    public Token next;
     ...
     * regular (non-special) token. If there are no such special tokens, this field is setClass to null. When there are
     * more than one such special token, this field refers to the last of these special tokens, which in turn refers to
     * the next previous special token through its specialToken field, and so on until the first special token (whose
