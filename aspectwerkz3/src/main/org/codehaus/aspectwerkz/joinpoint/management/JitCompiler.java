...
import org.codehaus.aspectwerkz.transform.delegation.AsmHelper;
    ...
    public static JoinPoint compileJoinPoint(final int joinPointHash, final int joinPointType,
        ...
        try {
            ...
            RttiInfo rttiInfo = setRttiInfo(
                    joinPointType, joinPointHash, declaringClass, system, targetInstance,
            ...
            if (joinPointClass == null) {
                ...
                createProceedMethod(
                        joinPointType, cw, className, declaringClass, joinPointHash, rttiInfo,
...
//                AsmHelper.dumpClass("_dump", className, cw);
    ...
    private static void createMemberFields(final int joinPointType, final ClassWriter cw, final String className) {
        ...
        switch (joinPointType) {
            ...
            case JoinPointType.METHOD_CALL:
                cw.visitField(
                        Constants.ACC_PRIVATE, SIGNATURE_FIELD_NAME, METHOD_SIGNATURE_IMPL_CLASS_SIGNATURE, null,
                        null
                );
            ...
            case JoinPointType.CONSTRUCTOR_EXECUTION:
                cw.visitField(
                        Constants.ACC_PRIVATE, SIGNATURE_FIELD_NAME, CONSTRUCTOR_SIGNATURE_IMPL_CLASS_SIGNATURE,
                        null, null
                );
                cw.visitField(
                        Constants.ACC_PRIVATE, RTTI_FIELD_NAME, CONSTRUCTOR_RTTI_IMPL_CLASS_SIGNATURE, null, null
                );
                break;
            case JoinPointType.FIELD_SET:
            case JoinPointType.FIELD_GET:
                cw.visitField(
                        Constants.ACC_PRIVATE, SIGNATURE_FIELD_NAME, FIELD_SIGNATURE_IMPL_CLASS_SIGNATURE, null,
                        null
                );
    ...
    private static boolean createInitMethod(
        ...
        CodeVisitor cv = cw.visitMethod(
                Constants.ACC_PUBLIC, INIT_METHOD_NAME, JIT_JOIN_POINT_INIT_METHOD_SIGNATURE,
                null, null
        );
        ...
        switch (joinPointType) {
            ...
            case JoinPointType.CONSTRUCTOR_EXECUTION:
                ...
                cv.visitFieldInsn(
                        Constants.PUTFIELD, className, RTTI_FIELD_NAME, CONSTRUCTOR_RTTI_IMPL_CLASS_SIGNATURE
                );
                break;
            case JoinPointType.FIELD_SET:
            case JoinPointType.FIELD_GET:
    ...
    private static void createGetSignatureMethod(
            final int joinPointType, final ClassWriter cw, final String className) {
        CodeVisitor cv = cw.visitMethod(
                Constants.ACC_PUBLIC, GET_SIGNATURE_METHOD_NAME,
    ...
    private static void createGetRttiMethod(final int joinPointType, final ClassWriter cw, final String className) {
        ...
        switch (joinPointType) {
            ...
            case JoinPointType.CONSTRUCTOR_EXECUTION:
                cv.visitFieldInsn(
                        Constants.GETFIELD, className, RTTI_FIELD_NAME, CONSTRUCTOR_RTTI_IMPL_CLASS_SIGNATURE
                );
                break;
            case JoinPointType.FIELD_SET:
            case JoinPointType.FIELD_GET:
    ...
    private static void createProceedMethod(
        ...
        CodeVisitor cv = cw.visitMethod(
                Constants.ACC_PUBLIC | Constants.ACC_FINAL, PROCEED_METHOD_NAME,
    ...
    private static void invokeJoinPoint(
            final int joinPointType, final Class declaringClass, final int joinPointHash,
    ...
    private static void invokeMethodExecutionJoinPoint(
        ...
        if (Modifier.isPublic(targetMethod.getModifiers()) && Modifier.isPublic(declaringClass.getModifiers())) {
            invokeMethod(
                    targetMethod, cv, joinPointType, argTypes, className, declaringClassName, methodName,
                    methodDescriptor
            );
        } else {
    ...
    private static void invokeConstrutorExecutionJoinPoint(
        ...
        if (Modifier.isPublic(targetConstructor.getModifiers()) && Modifier.isPublic(declaringClass.getModifiers())) {
            invokeConstructorExecution(
                    joinPointType, newArgTypes, cv, className, declaringClassName,
    ...
    private static void setReturnValue(final Method targetMethod, final CodeVisitor cv, final String className) {
        if (Type.getReturnType(targetMethod).getSort() != Type.VOID) {
            ...
            cv.visitVarInsn(Constants.ALOAD, 1);
            cv.visitMethodInsn(
                    Constants.INVOKEVIRTUAL, METHOD_RTTI_IMPL_CLASS_NAME, SET_RETURN_VALUE_METHOD_NAME,
    ...
    private static void setNewInstance(final CodeVisitor cv, final String className) {
        ...
        cv.visitVarInsn(Constants.ALOAD, 1);
        cv.visitMethodInsn(
                Constants.INVOKEVIRTUAL, CONSTRUCTOR_RTTI_IMPL_CLASS_NAME, SET_NEW_INSTANCE_METHOD_NAME,
    ...
    private static void setFieldValue(final CodeVisitor cv, final String className) {
        ...
        cv.visitVarInsn(Constants.ALOAD, 1);
        cv.visitMethodInsn(
                Constants.INVOKEVIRTUAL, FIELD_RTTI_IMPL_CLASS_NAME, SET_FIELD_VALUE_METHOD_NAME,
    ...
    private static void invokeMethod(
            final Method targetMethod, final CodeVisitor cv, final int joinPointType,
        ...
        if (!Modifier.isStatic(targetMethod.getModifiers())) {
            cv.visitVarInsn(Constants.ALOAD, 0);
            cv.visitFieldInsn(
                    Constants.GETFIELD, className, TARGET_INSTANCE_FIELD_NAME, WEAK_REFERENCE_CLASS_SIGNATURE
            );
            cv.visitMethodInsn(
                    Constants.INVOKEVIRTUAL, WEAK_REFERENCE_CLASS_NAME, WEAK_REFERENCE_GET_METHOD_NAME,
    ...
    private static void invokeMethodExecutionReflectively(final CodeVisitor cv) {
        cv.visitVarInsn(Constants.ALOAD, 0);
        cv.visitMethodInsn(
                Constants.INVOKESTATIC, JOIN_POINT_BASE_CLASS_NAME,
                INVOKE_TARGET_METHOD_EXECUTION_METHOD_NAME, INVOKE_TARGET_METHOD_EXECUTION_METHOD_SIGNATURE
        );
    }
    ...
    private static void invokeMethodCallReflectively(final CodeVisitor cv) {
        cv.visitVarInsn(Constants.ALOAD, 0);
        cv.visitMethodInsn(
                Constants.INVOKESTATIC, JOIN_POINT_BASE_CLASS_NAME, INVOKE_TARGET_METHOD_CALL_METHOD_NAME,
    ...
    private static void invokeConstructorCall(
            final int joinPointType, final Type[] argTypes, final CodeVisitor cv,
    ...
    private static void invokeConstructorCallReflectively(final CodeVisitor cv) {
        ...
        cv.visitMethodInsn(
                Constants.INVOKESTATIC, JOIN_POINT_BASE_CLASS_NAME,
                INVOKE_TARGET_CONSTRUCTOR_CALL_METHOD_NAME, INVOKE_TARGET_CONSTRUCTOR_CALL_METHOD_SIGNATURE
        );
    }
    ...
    private static void invokeConstructorExecution(
            final int joinPointType, final Type[] newArgTypes,
            final CodeVisitor cv, final String className,
    ...
    private static void invokeTargetFieldGet(final CodeVisitor cv) {
        cv.visitVarInsn(Constants.ALOAD, 0);
        cv.visitMethodInsn(
                Constants.INVOKESTATIC, JOIN_POINT_BASE_CLASS_NAME, GET_TARGET_FIELD_METHOD_NAME,
    ...
    private static void invokeTargetFieldSet(final CodeVisitor cv) {
        cv.visitVarInsn(Constants.ALOAD, 0);
        cv.visitMethodInsn(
                Constants.INVOKESTATIC, JOIN_POINT_BASE_CLASS_NAME, SET_TARGET_FIELD_METHOD_NAME,
    ...
    private static Labels invokeAdvice(
            final CodeVisitor cv, final String className, final IndexTuple[] aroundAdvices,
        ...
        invokeBeforeAfterAdvice(
                hasBeforeAfterAdvice, beforeAdvices, afterAdvices, className, cv, switchCaseLabels,
                returnLabels
        );
    ...
    private static void invokeBeforeAfterAdvice(
            boolean hasBeforeAfterAdvice, final IndexTuple[] beforeAdvices,
    ...
    private static void invokesAroundAdvice(
            boolean hasBeforeAfterAdvice, final IndexTuple[] aroundAdvices,
    ...
    private static void prepareParameterUnwrapping(
            final int joinPointType, final Type[] argTypes,
            final CodeVisitor cv, final String className) {
        if (argTypes.length != 0) {
            ...
            switch (joinPointType) {
                ...
                case JoinPointType.METHOD_CALL:
                    cv.visitFieldInsn(
                            Constants.GETFIELD, className, RTTI_FIELD_NAME, METHOD_RTTI_IMPL_CLASS_SIGNATURE
                    );
                    cv.visitMethodInsn(
    ...
    private static RttiInfo setRttiInfo(
            final int joinPointType, final int joinPointHash, final Class declaringClass,
            final AspectSystem system, final Object thisInstance,
