...
public class AspectC
{
        ...
        final String destDir)
    {
        ...
        for (int i = 0; i < classNames.length; i++)
        {
            ...
            if (enhancer.initialize(className, classPath))
            {
                ...
                {
                    JavaClass javaClass = parseClassAttributes(qdoxParser,
                            enhancer);
        ...
        final AttributeEnhancer enhancer)
    {
        ...
        for (int i = 0; i < tags.length; i++)
        {
            ...
            enhancer.insertFieldAttribute(javaField,
                new CustomAttribute(name, value, null));
            log("field [" + javaField.getParentClass().getFullyQualifiedName()
                + '.' + javaField.getName() + ']');
            log("\tattribute [" + name + ' ' + value + ']');
        }
        ...
        final AttributeEnhancer enhancer)
    {
        ...
        if (aspectTag != null)
        {
            ...
            enhancer.insertClassAttribute(new AspectAttribute(name,
                    deploymentModel));
     ...
     * @param enhancer  the attribute enhancer
     */
    private void parseExpressionPointcut(final JavaField javaField,
        final AttributeEnhancer enhancer)
    {
        ...
        final AttributeEnhancer enhancer)
    {
        ...
        log("\tinterface introduction [" + javaField.getType().getValue()
            + "::" + expression + ']');
    }
     ...
     * @param enhancer
     */
    private void parseIntroduction(final JavaClass innerClass,
        final AttributeEnhancer enhancer)
    {
        ...
        for (int i = 0; i < introductionTags.length; i++)
        {
            ...
            String deploymentModel = introductionTag.getNamedParameter(
                    "deploymentModel");
            StringBuffer buf = new StringBuffer();
            ...
            if (introducedInterfaceNames.length == 0)
            {
                introducedInterfaceNames = enhancer
                    .getNearestInterfacesInHierarchy(innerClass
                        .getFullyQualifiedName());
                ...
                {
                    log("\tintroduction introduce implicit ["
            ...
            log("\tintroduction impl [" + innerClass.getName() + "::"
                + expression + "] " + deploymentModel);
        }
    }
    ...
    {
        return tag.getName().startsWith(ATTR_GENERIC_PREFIX1)
        || tag.getName().startsWith(ATTR_GENERIC_PREFIX2);
    }
    ...
    public static void main(String[] args)
    {
        ...
        {
            System.out.println(
                "AspectWerkz (c) 2002-2004 Jonas Bonér, Alexandre Vasseur");
            System.out.println(
        ...
        if (arguments.size() == 2)
        {
            compiler.compile((String) arguments.get(0),
                (String) arguments.get(1));
            compiler.log("compiled classes written to "
                + (String) arguments.get(1));
        }
        else
        {
