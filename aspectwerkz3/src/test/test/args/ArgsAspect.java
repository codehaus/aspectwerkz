...
public class ArgsAspect {
    ...
    void pc_getFirst(String s) {;}// here we use "return void" style

    /** @Expression in_scope && execution(* changeArg(..)) && args(String, s, long) */
    Pointcut pc_changeArg(StringBuffer s) {return null;}// here we use "return null" style

    /** @Expression in_scope && execution(* orderChangedInPointcutSignature(..)) && args(s0, s1, long) */
    Pointcut pc_orderChangedInPointcutSignature(String s1, String s0) {return null;}

    /** @Expression in_scope && execution(* orderChangedInAdviceSignature(..)) && args(s0, s1, long) */
    Pointcut pc_orderChangedInAdviceSignature(String s0, String s1) {return null;}

    /** @Expression in_scope && execution(* orderChangedInPointcutAndAdviceSignature(..)) && args(s0, s1, long) */
    Pointcut pc_orderChangedInPointcutAndAdviceSignature(String s1, String s0) {return null;}
    ...
    public Object orderChangedInPointcutAndAdviceSignatureAround(JoinPoint joinPoint, String as0, String as1) throws Throwable {
        ((Loggable)joinPoint.getTarget()).log("before1 " + as0 + " " + as1 + " ");
        Object res = joinPoint.proceed();
        ((Loggable)joinPoint.getTarget()).log("after1 " + as0 + " " + as1 + " ");
        return res;
